...
SELECT CASE WHEN @v_affected = 0
    THEN CONCAT('-40,Migration with Id [', IFNULL(CAST(@MigrationId AS CHAR), 'NULL'), '] does not exist')
    ELSE CONCAT(CAST(@MigrationId AS CHAR), ',Migration with Id [', CAST(@MigrationId AS CHAR), '] updated: Block [', CAST(@FileUpBlocksMigrated AS CHAR), '] - Status [', CAST(@MigrationStatusId AS CHAR), ']')
END;
