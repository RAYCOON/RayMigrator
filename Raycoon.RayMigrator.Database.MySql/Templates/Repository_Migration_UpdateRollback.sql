...
Note4 = "MigrationStatusId values: 10=Pending, 20=Executing, 30=Failed, 50=NotMigrated, 100=Migrated"
================================================================================
*/
...
WHERE id = @MigrationId AND @MigrationStatusId IN (100, 50, 30) AND @v_affected > 0;

SELECT CASE WHEN @v_affected = 0
    THEN CONCAT('-1,Migration with Id [', IFNULL(CAST(@MigrationId AS CHAR), 'NULL'), '] does not exist')
    ELSE CONCAT(CAST(@MigrationId AS CHAR), ',Migration with Id [', CAST(@MigrationId AS CHAR), '] rollback updated: Block [', CAST(@FileDownBlocksMigrated AS CHAR), '/', CAST(@FileDownBlocksTotal AS CHAR), '] - Status [', CAST(@MigrationStatusId AS CHAR), ']')
END;
