...
SET @v_exists = (SELECT COUNT(*) FROM {CFG:TableBaseName}migration_record WHERE id = @MigrationId);
...
WHERE id = @MigrationId AND @v_exists > 0;

SELECT CASE WHEN @v_exists = 0
    THEN CONCAT('-1,Migration with Id [', IFNULL(CAST(@MigrationId AS CHAR), 'NULL'), '] does not exist')
    ELSE CONCAT(CAST(@MigrationId AS CHAR), ',Migration with Id [', CAST(@MigrationId AS CHAR), '] hashes updated')
END;
