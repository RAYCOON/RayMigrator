...
2. **Array handling differs between appsettings and migsettings**: In the migsettings hierarchy, arrays (`Environments` and `Targets`) are completely replaced by the overriding layer, not merged with the parent. In appsettings file merging, arrays whose elements carry an `Alias` (`Products`, `TargetGroups`, `Targets`, `CliTools`) are merged **by alias**: a later file overrides the element with the same alias property by property, new aliases are appended, elements the later file does not mention are kept, and position never matters. All other appsettings arrays are replaced as a whole by the later file. See [Arrays in Configuration Hierarchy](appsettings-hierarchy.md#arrays); until [#23](https://github.com/RAYCOON/RayMigrator/issues/23) ships the engine still merges by position, so repeat arrays completely and in the same order.
