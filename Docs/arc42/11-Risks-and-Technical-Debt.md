...
| R-14 | An environment or product file lists `Products` (or nested `TargetGroups` / `Targets`) in another order than the base file, or names only the element it changes; the engine merges arrays by position and attaches the override, including target groups, to the wrong product. Reproduced: `migrate-up` of one product migrated the other product's database with exit `0`. | Functional correctness and integrity | High | High | Until #23 ships: repeat every array completely and in the same order (interim note on the configuration pages); `RULE_1_8` catches only the case where the alias becomes duplicated | Fix decided in ADR-021, tracked in #23 with unit tests; the Config Wizard already merges by alias and shows the intended result |
...
|----|------|----------------|----------------------|
| TD-D-16 | Until issue #23 ships, the engine merges `Products`, `TargetGroups`, `Targets` and `CliTools` by position while the documentation and the Config Wizard describe the decided alias merge (ADR-021); the configuration pages carry an interim note. | `06-configuration-reference/appsettings-hierarchy.md`, `06-configuration-reference/settings-inheritance-overview.md`, `arc42/03-Context-and-Scope.md` | Implement #23 with its unit tests, then remove the interim notes and the "until #23" clauses |
