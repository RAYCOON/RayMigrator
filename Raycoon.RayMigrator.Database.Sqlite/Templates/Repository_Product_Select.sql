...
SELECT CASE
    WHEN (SELECT "Id" FROM "{CFG:TableBaseName}Product" WHERE "NameLower" = @NameLower LIMIT 1) IS NULL
        THEN '0,Product [' || IFNULL(@Name, 'NULL') || '] not found'
    ELSE CAST((SELECT "Id" FROM "{CFG:TableBaseName}Product" WHERE "NameLower" = @NameLower LIMIT 1) AS TEXT)
         || ',Product [' || @Name || '] with Id ['
         || CAST((SELECT "Id" FROM "{CFG:TableBaseName}Product" WHERE "NameLower" = @NameLower LIMIT 1) AS TEXT) || '] found'
END;
