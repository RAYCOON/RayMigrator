...
SELECT COUNT(*) FROM "{CFG:TableBaseName}MigrationRecord" WHERE "Id" = @MigrationId;
...
WHERE "Id" = @MigrationId AND (SELECT "cnt" FROM "_rc_exists") > 0;
...
SELECT CASE WHEN (SELECT "cnt" FROM "_rc_exists") = 0
    ...
    ELSE CAST(@MigrationId AS TEXT) || ',Migration with Id [' || CAST(@MigrationId AS TEXT) || '] hashes updated'
END;
