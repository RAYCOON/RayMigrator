...
SELECT CASE WHEN (SELECT "cnt" FROM "_rc_affected") = 0
    THEN '-40,Migration with Id [' || IFNULL(CAST(@MigrationId AS TEXT), 'NULL') || '] does not exist'
    ELSE CAST(@MigrationId AS TEXT) || ',Migration with Id [' || CAST(@MigrationId AS TEXT) || '] updated: Block [' || CAST(@FileUpBlocksMigrated AS TEXT) || '] - Status [' || CAST(@MigrationStatusId AS TEXT) || ']'
END;
