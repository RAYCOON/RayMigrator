...
SELECT CASE WHEN (SELECT "cnt" FROM "_rc_affected") = 0
    ...
    ELSE CAST(@MigrationRecordId AS TEXT) || ',Migration with Id [' || CAST(@MigrationRecordId AS TEXT) || '] rollback updated: Block [' || CAST(@FileDownBlocksMigrated AS TEXT) || '/' || CAST(@FileDownBlocksTotal AS TEXT) || '] - Status [' || CAST(@MigrationStatusId AS TEXT) || ']'
END;
