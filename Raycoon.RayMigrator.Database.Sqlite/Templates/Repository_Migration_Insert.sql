...
[Parameters]
ExistingMigrationId  = "INTEGER | REQUIRED | 0 = INSERT new record, >0 = UPDATE existing record with this ID"
...
Note5 = "Uses last_insert_rowid() to retrieve the new MigrationId"
...
SET
    ...
    "FinishedAt"              = NULL,
    "DurationInMs"            = NULL
WHERE "Id" = @ExistingMigrationId AND @ExistingMigrationId > 0;
...
SELECT
    ...
    @MigrateDownFileExists,
    datetime('now')
WHERE @ExistingMigrationId <= 0 OR @ExistingMigrationId IS NULL;

SELECT CASE
    WHEN @ExistingMigrationId > 0 THEN
        CAST(@ExistingMigrationId AS TEXT) || ',Migration record with Id [' || CAST(@ExistingMigrationId AS TEXT) || '] successfully reset for file [' || IFNULL(@Filename, 'NULL') || ']'
    ELSE
