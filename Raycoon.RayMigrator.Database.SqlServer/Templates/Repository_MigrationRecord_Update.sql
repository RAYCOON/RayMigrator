...
BEGIN TRY
    ...
    IF (@v_affected = 0)
    BEGIN
        SELECT '-40,Migration with Id [' + COALESCE(CAST(@MigrationRecordId AS VARCHAR(10)), 'NULL') + '] does not exist';
        RETURN;
    END;
    ...
    SELECT CAST(@MigrationRecordId AS VARCHAR(10)) + ',Migration with Id [' + CAST(@MigrationRecordId AS VARCHAR(10)) + '] updated: Block [' + CAST(@FileUpBlocksMigrated AS VARCHAR(10)) + '] - Status [' + CAST(@MigrationStatusId AS VARCHAR(10)) + ']';
