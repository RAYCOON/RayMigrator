...
BEGIN TRY
    ...
    IF NOT EXISTS (SELECT 1 FROM [{CFG:SchemaName}].[{CFG:TableBaseName}MigrationRecord] WHERE Id = @MigrationRecordId)
    BEGIN
        SELECT '-1,Migration with Id [' + COALESCE(CAST(@MigrationRecordId AS VARCHAR(10)), 'NULL') + '] does not exist';
        RETURN;
    END;
    ...
    SELECT CAST(@MigrationRecordId AS VARCHAR(10)) + ',Migration with Id [' + CAST(@MigrationRecordId AS VARCHAR(10)) + '] hashes updated';
