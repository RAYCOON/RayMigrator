...
ExistingMigrationId = "INT | REQUIRED | 0 = INSERT new record, >0 = UPDATE existing record with this ID"
...
Success_Updated = "N (MigrationId),Migration record with Id [N] successfully reset for file [Filename]"

[ModificationNotes]
Note1 = "SELECT result format: 'code,message' - DO NOT change this format"
Note2 = "No commas allowed in error messages"
Note3 = "Use SYSUTCDATETIME() for StartedAt timestamp"
Note4 = "FileUpBlocksMigrated initialized to 0 - updated by Repository_Migration_Update"
...
BEGIN TRY
    ...
    BEGIN
        ...
        SET
            ...
            FinishedAt              = NULL,
            DurationInMs            = NULL
        WHERE Id = @ExistingMigrationId;

        SET @MigrationId = @ExistingMigrationId;

        SELECT CAST(@MigrationId AS VARCHAR(10)) + ',Migration record with Id [' + CAST(@MigrationId AS VARCHAR(10)) + '] successfully reset for file [' + COALESCE(@Filename, 'NULL') + ']';
    END
    ELSE
    BEGIN
        -- Insert new record
        ...
        SELECT CAST(@MigrationId AS VARCHAR(10)) + ',Migration record with Id [' + CAST(@MigrationId AS VARCHAR(10)) + '] successfully created for file [' + COALESCE(@Filename, 'NULL') + ']';
    END
