...
BEGIN
    ...
    IF v_affected = 0 THEN
        RAISE EXCEPTION '-40,Migration with Id [%] does not exist', COALESCE(CAST(@MigrationRecordId AS VARCHAR(10)), 'NULL');
    END IF;
    ...
    RAISE NOTICE '%,Migration with Id [%] updated: Block [%] - Status [%]',
        @MigrationRecordId, @MigrationRecordId, @FileUpBlocksMigrated, @MigrationStatusId;
END $$;
