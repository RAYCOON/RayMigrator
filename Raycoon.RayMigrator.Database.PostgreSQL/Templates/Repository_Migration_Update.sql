...
BEGIN
    ...
    IF v_affected = 0 THEN
        RAISE EXCEPTION '-40,Migration with Id [%] does not exist', COALESCE(CAST(@MigrationId AS VARCHAR(10)), 'NULL');
    END IF;
    ...
    RAISE NOTICE '%,Migration with Id [%] updated: Block [%] - Status [%]',
        @MigrationId, @MigrationId, @FileUpBlocksMigrated, @MigrationStatusId;
END $$;
