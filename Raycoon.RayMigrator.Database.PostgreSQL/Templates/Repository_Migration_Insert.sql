...
DECLARE
    v_migration_id INT;
    v_existing_id INT := @ExistingMigrationId;
BEGIN
    IF v_existing_id > 0 THEN
        ...
        v_migration_id := v_existing_id;

        RAISE NOTICE '%,Migration record with Id [%] successfully reset for file [%]', v_migration_id, v_migration_id, COALESCE(@Filename, 'NULL');
    ELSE
        -- Insert new record
        ...
        RAISE NOTICE '%,Migration record with Id [%] successfully created for file [%]', v_migration_id, v_migration_id, COALESCE(@Filename, 'NULL');
    END IF;
END $$;
