...
public class MariaDbCliToolExitCodeRangeTests : MariaDbTestBase
{
    public MariaDbCliToolExitCodeRangeTests(MariaDbFixture fixture) : base(fixture) { }
    ...
    public async Task RangeIncludesZero_ToolReturnsZero_MigrationSucceeds()
    {
        ...
        var cfg = CliToolConfigHelper.GetStdinConfig("MariaDb", Fixture.EngineConfig.ConnectionString);

        // "0..5" is a closed range that includes 0 — mariadb returns 0 on success, so this should work
    ...
    public async Task RangeExcludesZero_ToolReturnsZero_MigrationFails()
    {
        ...
        // "1..5" does NOT include 0 — mariadb returns 0, but the whitelist rejects it
        await using var ctx = await CreateScenario()
            .WithCliTool(cfg.Alias, cfg.ExecutablePath, cfg.ArgumentTemplate, cfg.InputMode,
