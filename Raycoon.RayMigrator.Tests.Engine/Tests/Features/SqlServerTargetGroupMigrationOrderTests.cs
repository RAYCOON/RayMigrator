...
public class SqlServerTargetGroupMigrationOrderTests : SqlServerTestBase
{
    public SqlServerTargetGroupMigrationOrderTests(SqlServerFixture fixture) : base(fixture) { }

    /// <summary>
    /// Copies SqlServer_Frontend migration files into the work directory so the engine
    ...
    private static void CopyFrontendFiles(string workDir)
    {
        string frontendBase = Path.Combine(
            AppContext.BaseDirectory, "MigrationFiles", "SqlServer_Frontend");
    ...
    public async Task MigrateUp_CliOverridesAppSettings()
    {
        ...
        // CLI explicitly passes Backend first, overriding appsettings Frontend first
        var result = await ctx.MigrateUpAsync(
    ...
    public async Task MigrateUp_MigSettingsOrder_FrontendBeforeBackend()
    {
        ...
        // Migrate only Release_1.0 so we can check the single-release order
    ...
    public async Task MigrateUp_SingleTargetGroup_ShouldFailWhenOrderSpecified()
    {
        ...
        // No Frontend target group — only Backend exists
        var result = await ctx.MigrateUpAsync(
