...
public class SqliteMigrationRunMetaTests : SqliteTestBase
{
    public SqliteMigrationRunMetaTests(SqliteFixture fixture) : base(fixture) { }
    ...
    public async Task Product_WithTargetGroups_InJson()
    {
        ...
        targetGroups.GetArrayLength().Should().BeGreaterThan(0, "Product should have at least one TargetGroup");
        foreach (var tg in targetGroups.EnumerateArray())
        {
            tg.TryGetProperty("Targets", out var targets).Should().BeTrue();
            targets.GetArrayLength().Should().BeGreaterThan(0, "Each TargetGroup should have at least one Target");
        }
    }
    ...
    public async Task MetaCount_MatchesRunCount()
    {
        ...
        metaCount.Should().Be(runCount, "MigrationRunMeta should have a 1:1 relationship with MigrationRun");
    }
